...
from kms.core import clustering, embeddings, models, reranker
...
def test_assign_local_hubs_reranks_before_choose_hub(monkeypatch):
    # assign_local_hubs sorts candidate hubs by cosine similarity, then reranks
    # them, then hands the reranked list to _choose_hub. The reranker here swaps
    # the cosine order [Auto, Beta, Alpha] into [Alpha, Beta, Auto] so that two
    # boundary candidates (judge-rejected Alpha, judge-approved Beta) are
    # examined before the highest-cosine hub Auto that would otherwise
    # auto-merge. _choose_hub therefore returns Beta -- chosen by the judge over
    # a higher-cosine auto-merge candidate -- proving rerank ordering flows into
    # hub selection and the judge (not raw similarity) settles membership.
    fake_settings = SimpleNamespace(
        stages=SimpleNamespace(
            search=SimpleNamespace(top_k=5),
    ...
    monkeypatch.setattr(embeddings, 'embedder', lambda: _LocalEmbedder())

    reranker_fake = _FakeReranker(index_order=[2, 1, 0])
    _patch_reranker(monkeypatch, configured=True, fake=reranker_fake)

    component = models.HubRecord(
        uuid='comp-c1',
        name='comp',
    ...
    candidate_hubs = (
        models.HubRecord(
            uuid='hub-auto',
            name='Auto Hub',
            description='auto hub description',
            embedding=[1.0, 0.1],
            source='s',
            aliases=(),
        ),
        models.HubRecord(
            uuid='hub-beta',
            name='Beta Hub',
            description='beta hub description',
            embedding=[1.0, 0.5],
            source='s',
            aliases=(),
        ),
        models.HubRecord(
            uuid='hub-alpha',
            name='Alpha Hub',
            description='alpha hub description',
            embedding=[1.0, 1.0],
            source='s',
            aliases=(),
        ),
    ...
    adjudicator = _LocalAdjudicator()
    synthesizer = _LocalSynthesizer()
    result = asyncio.run(
        local_entity_hubs.assign_local_hubs(
            bundle,
            max_concurrency=8,
    ...
    # Cosine sort feeds the reranker the hubs in [Auto, Beta, Alpha] order; the
    # reranker reorders them to [Alpha, Beta, Auto].
    assert reranker_fake.calls[0][1] == [
        'Auto Hub auto hub description',
        'Beta Hub beta hub description',
        'Alpha Hub alpha hub description',
    ]
    # _choose_hub consumes the reranked order: it adjudicates Alpha (rejected)
    # then Beta (approved) and never reaches Auto.
    assert adjudicator.calls == ['Alpha Hub', 'Beta Hub']
    # The judge-approved Beta hub (lower cosine) wins over Auto (higher cosine).
    assert result['assignments'] == [
        {'component': 'comp-c1', 'hub': 'hub-beta'}
    ]
    assert result['assigned'] == 1
    assert result['new_hubs'] == 0
    assert result['changed_hubs'] == ['hub-beta']
    # The full assignment path ran hub synthesis for the chosen hub.
    assert len(synthesizer.calls) == 1


class _LocalAdjudicator:
    def __init__(self):
        self.calls = []

    async def aforward(self, left, right, scope):
        self.calls.append(right.name)
        return right.name == 'Beta Hub'


class _RejectingAdjudicator:
    """Adjudicator that rejects every comparison."""
...
class _LocalSynthesizer:
    ...
    async def aforward(self, surface_forms, descriptions, scope):
        ...
        return ('synthesized hub', 'a synthesized description')
...
@pytest.mark.parametrize(
    'hub_module',
    [
        pytest.param(local_entity_hubs, id='entity'),
        pytest.param(local_predicate_hubs, id='predicate'),
    ],
)
def test_assign_local_hubs_pruning_excludes_omitted(hub_module, monkeypatch):
    # assign_local_hubs score-sorts candidate hubs by cosine similarity, then
    # reranks them, then hands the selected subset to _choose_hub. The reranker
    # below returns only two of three candidates, omitting High (the auto-merge
    # candidate). Without pruning, High would auto-merge (cosine 0.995 >=
    # 0.9); with pruning it is excluded entirely and a new hub is created
    # instead. Every selected candidate is adjudicated (and rejected), while the
    # omitted one never reaches the adjudicator.
    fake_settings = SimpleNamespace(
        stages=SimpleNamespace(
            search=SimpleNamespace(top_k=5),
            entity_hubs=SimpleNamespace(
                merge_above=0.9, separate_below=0.2, rerank_top_n=2
            ),
    ...
    reranker_fake = _FakeReranker(index_order=[2, 1])
    _patch_reranker(monkeypatch, configured=True, fake=reranker_fake)
    ...
    candidate_hubs = (
        models.HubRecord(
            uuid='hub-high',
            name='High Hub',
            description='high hub description',
            embedding=[1.0, 0.1],
            source='s',
            aliases=(),
        ),
        models.HubRecord(
            uuid='hub-med',
            name='Med Hub',
            description='med hub description',
            embedding=[1.0, 0.5],
            source='s',
            aliases=(),
        ),
        models.HubRecord(
            uuid='hub-low',
            name='Low Hub',
            description='low hub description',
            embedding=[1.0, 1.0],
            source='s',
            aliases=(),
        ),
    ...
    adjudicator = _RejectingAdjudicator()
    synthesizer = _LocalSynthesizer()
    result = asyncio.run(
        hub_module.assign_local_hubs(
            bundle,
            max_concurrency=8,
            adjudicator=adjudicator,
            synthesizer=synthesizer,
    ...
    # Cosine sort: High (0.98), Med (0.89), Low (0.71).
    # The reranker reorders to [Low, Med], omitting High entirely.
    assert reranker_fake.calls[0][1] == [
        'High Hub high hub description',
        'Med Hub med hub description',
        'Low Hub low hub description',
    ]
    # Only the reranker-selected candidates reach the adjudicator; the omitted
    assert adjudicator.calls == ['Low Hub', 'Med Hub']
    # No merge occurred (all selected candidates rejected, and the auto-merge
    # candidate High was pruned), so a new hub is created.
    assert result['new_hubs'] == 1
    assert result['assigned'] == 1
    # The component is assigned to a new hub, not to any omitted candidate.
    assert result['assignments'][0]['component'] == 'comp-c1'
    assert result['assignments'][0]['hub'] not in {
        'hub-high',
        'hub-med',
        'hub-low',
    }
    # The new hub was synthesized (once in _new_hub, once in _refresh_local_hubs).
...
def test_adjudicated_groups_logs_adjudication_decisions(caplog):
    ...
    adjudication_logs = [
        m for m in messages if 'clustering adjudication:' in m
    ]
