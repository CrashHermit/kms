...
and that a core.models.ASTNode maps onto the expected Neo4j property shape."""
...
def test_node_uuid_distinguishes_index_and_source():
    assert node_uuid('hefferon.pdf', 7) != node_uuid(
        'hefferon.pdf', 8
    )  # different position
    assert node_uuid('hefferon.pdf', 7) != node_uuid(
        'lebl.pdf', 7
...
def test_node_properties_maps_type_content_and_provenance():
    node = models.ASTNode(
        type=models.NodeType.MATH, content='$x^2$', id=3, segment_index=2
    )
...
def test_node_label_is_the_capitalized_type():
    assert node_label(models.ASTNode(type=models.NodeType.MATH)) == 'Math'
    assert (
...
def test_node_label_is_none_without_a_type():
    assert node_label(models.ASTNode()) is None


def test_node_properties_link_back_to_the_source_node():
    node = models.ASTNode(
        type=models.NodeType.MATH, content='$x$', id=3, segment_index=2
    )
    assert node_properties(node, 'book.pdf')['source'] == source_uuid(
        'book.pdf'
    )
...
def test_source_properties_carry_key_and_uuid_and_merge_metadata():
    props = source_properties(
        'book.pdf', {'title': 'Linear Algebra', 'author': 'Hefferon'}
    )
...
def test_source_metadata_cannot_clobber_key_or_uuid():
    ...
    assert (
        props['uuid'] == source_uuid('book.pdf') and props['key'] == 'book.pdf'
    )
