...
def test_orchestrator_persists_only_verified_cards(monkeypatch):
    candidate = EntityLearningCandidate(
        global_hub=EntityLearningGlobalContext(
            uuid='g', name='G', description='context'
        ),
        local_hub=EntityLearningCandidateInput(
            index=1, local_uuid='l', name='N', description='local'
        ),
        source='book',
        evidence=(
            EntityLearningEvidenceInput(
                index=1, uuid='e', name='N', description='fact'
            ),
        ),
    )
    assessment = EntityLearningAssessment(
        candidates=(candidate,),
        global_decisions=(True,),
    ...
    monkeypatch.setattr(
        cards.writer,
        'persist_cards',
        lambda items, **kwargs: _async_collect(persisted, items),
    )
    monkeypatch.setattr(
        cards.writer,
        'persist_card_hub_edges',
        lambda items, **kwargs: _async_noop(),
    )
    monkeypatch.setattr(
        cards.writer,
        'persist_card_evidence',
        lambda items, **kwargs: _async_noop(),
    )
    result = asyncio.run(
        cards.generate_entity_cards(
            session_factory=object(),
            global_gate=object(),
            evidence_gate=object(),
            router=_Router(),
            fact_generator=_Facts(),
            card_generator=_Generator(),
            verifier=_Verifier(),
        )
    )
