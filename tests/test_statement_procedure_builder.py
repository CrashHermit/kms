...
def _nodes():
    return {
        ...
        1: models.SourceNode(
            type='paragraph', content='Integrate ...', uuid='node-1'
        ),
        2: models.SourceNode(
            type='paragraph', content='Hence the value is 4.', uuid='node-2'
        ),
    }
...
def test_assigns_statement_ids_and_dual_role_links():
    nodes = {
        0: models.SourceNode(
            type='paragraph', content='Example', uuid='node-0'
        ),
        1: models.SourceNode(
            type='paragraph', content='Solution', uuid='node-1'
        ),
    }
...
def test_creates_a_hub_per_role():
    nodes = [
        models.SourceNode(
            type='paragraph', content='Theorem 2.1', uuid='node-0'
        ),
        models.SourceNode(
            type='paragraph', content='Proof. ...', uuid='node-1'
        ),
        models.SourceNode(
            type='paragraph', content='Exercise 3', uuid='node-2'
        ),
    ]
...
def test_neither_role_is_skipped():
    nodes = [
        models.SourceNode(
            type='header', content='Learning Objectives', uuid='node-0'
        ),
        models.SourceNode(
            type='paragraph', content='Theorem 2.1', uuid='node-1'
        ),
    ]
...
def test_a_both_block_creates_both_independent_hubs():
    nodes = [
        ...
        models.SourceNode(
            type='paragraph', content='The value is 4.', uuid='node-1'
        ),
    ]
    by_id = {i: node for i, node in enumerate(nodes)}
    role_mod = _ScriptedRoles([(True, True)])
...
def test_node_run_writes_the_hub_channels():
    ...
    assert statements[0].uuid == identity.statement_uuid('book.pdf', [0])
        ...
        node.run(
            {
                'nodes': [
                    models.SourceNode(
                        type='paragraph', content='x', uuid='node-0'
                    )
                ],
                'spans': [],
            }
