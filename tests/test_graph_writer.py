...
_STREAM = [
    ...
    models.ASTNode(
        type=models.NodeType.PARAGRAPH, content='b', id=3, segment_index=0
    ),
]
...
def test_batches_group_by_per_type_label():
    ...
    assert (
        len(batches['Paragraph']) == 2
    )  # two paragraphs share the label bucket
...
def test_typeless_node_falls_into_the_none_bucket():
    batches = node_batches(
        [models.ASTNode(content='?', id=0, segment_index=0)], 'book.pdf'
    )
...
def test_next_pairs_thread_consecutive_nodes_in_order():
    ...
    assert pairs[0] == {
        'from': node_uuid('book.pdf', 0),
        'to': node_uuid('book.pdf', 1),
    }
    assert pairs[-1] == {
        'from': node_uuid('book.pdf', 2),
        'to': node_uuid('book.pdf', 3),
    }
...
def test_persist_nodes_writes_source_nodes_head_and_next(monkeypatch):
    ...
    asyncio.run(
        persist_nodes(_STREAM, 'book.pdf', {'title': 'T', 'author': 'A'})
    )
    ...
    assert (
        calls[0][1]['props']['title'] == 'T'
        and calls[0][1]['props']['author'] == 'A'
    )
