...
def _bundle() -> models.ConstructionBundle:
    ...
    statement = models.Statement(block=[0, 1], members=[0, 1])
    procedure = models.Procedure(block=[0, 1], members=[0, 1])
...
def test_compose_procedure_rejects_bad_members_and_steps() -> None:
    with pytest.raises(ValueError, match='duplicate member positions'):
        compose_procedure(
            _bundle(), models.Procedure(block=[1], members=[0, 0])
        )
...
def test_compose_procedure_rejects_ambiguous_statement_linkage() -> None:
    ...
    bundle.statements.append(models.Statement(block=[0, 1], members=[2]))
