...
def test_event_card_creation_uses_event_evidence():
    ...
    class Facts:
        async def aforward(self, **kwargs):
            return [
                event_cards.EventCardFact(
                    text='The event occurred.', evidence_indexes=[1]
                )
            ]
    ...
    class Generator:
        async def aforward(self, **kwargs):
            return [
                event_cards.EventCardDraft(
                    prompt='What occurred?',
                    response='The event occurred.',
                    fact_index=1,
                )
            ]
    ...
    candidate = event_cards.EventCardCandidateInput(
        global_hub={'uuid': 'g'},
        local_hub={'name': 'Battle', 'local_uuid': 'h'},
        source='book',
        evidence=[
            {
                'index': 1,
    ...
    cards = asyncio.run(
        event_cards.create_event_cards(
            candidate,
            router=Router(),
