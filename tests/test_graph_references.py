...
def test_canonical_batches_dedupe_by_uuid_and_group_by_type_label():
    ...
    assert batches['Definition'][0]['uuid'] == canonical_uuid(
        'definition', 'Set'
    )
    assert (
        batches['Definition'][0]['type'] == 'definition'
    )  # typed like a mention definition
    assert batches['Theorem'][0]['uuid'] == canonical_uuid(
