...
Definition 2.5.1 (Primitive Root).
...
**Proposition 2.5.3** (Root Bound).

*Let $f \in k[x]$ be a nonzero polynomial over a field $k$. Then there are at most $\deg(f)$ elements $\alpha \in k$ such that $f(\alpha) = 0$.*
...
*SAGE Example 2.5.4.* We use Sage to find the roots of a polynomial over $\mathbf{Z}/13\mathbf{Z}$.
...
Recall from Section 2.1.2 that the order of an element $x$ in a finite group is the smallest $m \ge 1$ such that $x^m = 1$. In this section, we prove that $(\mathbf{Z}/p\mathbf{Z})^*$ is cyclic by using the results of Section 2.5.1 to produce an element of $(\mathbf{Z}/p\mathbf{Z})^*$ of order $d$ for each prime power divisor $d$ of $p - 1$, and then we multiply these together to obtain an element of order $p - 1$.
