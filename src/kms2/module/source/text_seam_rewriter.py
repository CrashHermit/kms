"""DSPy module for rewriting text seams."""
...
class TextSeamRewriteSignature(dspy.Signature):
    ...
    Delimiters, fences and environment begin/end pairs that only mark where the
    page ended are yours to remove, add or move so the result is well-formed.
    Judge what the block needs. The content between them is not yours to touch.
