"""Persist typed event enrichment without changing graph topology."""
...
class EventEnrichmentPersistenceNode:
    ...
    async def run(self, state: SemanticState) -> dict[str, int]:
        ...
        if state.event_embedding_results:
            await self._repository.update_event_enrichment(
                state.source_uuid,
                state.event_embedding_results,
            )
        return {'event_persisted_count': len(state.event_embedding_results)}
