"""Filter, synthesize, and stage source-local procedure hubs."""
...
from kms2.module.semantic.source_procedure_hub import SourceProcedureHubModule

logger = logging.getLogger(__name__)


class SourceProcedureHubNode:
    """Filter procedure pairs, build communities, and stage procedure hubs."""
    ...
    async def run(self, state: SemanticState) -> dict[str, object]:
        """Filter procedure pairs, detect communities, and stage hubs."""
        ...
        return {
            ...
            'source_procedure_hub_memberships': memberships,
        }

    async def _accepted_pairs(
        self, candidates: list[SourceProcedureHubCandidate]
    ) -> list[SourceProcedureHubCandidate]:
        """Apply reranker thresholds and procedure membership judgments."""
        ...
        logger.info(
            'source procedure hub accepted pairs: %d/%d',
            len(unique),
            len(candidates),
        )
    ...
    async def _judge_borderline(
        ...
        accepted: list[SourceProcedureHubCandidate] = []
        batch: list[SourceProcedureHubCandidate] = []
        batch_cost = 0
        for candidate in candidates:
    ...
    async def _judge_batch(
        self, batch: list[SourceProcedureHubCandidate]
    ) -> list[SourceProcedureHubCandidate]:
...
__all__ = ['SourceProcedureHubNode']
