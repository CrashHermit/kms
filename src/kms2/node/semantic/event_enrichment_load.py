"""Load and project source-local event enrichment requests."""
...
class EventEnrichmentLoadNode:
    ...
    async def run(self, state: SemanticState) -> dict[str, object]:
        ...
        occurrences = await self._semantic_repository.load_events(
            state.source_uuid
        )
        ordered = sorted(
            occurrences,
        ...
        for occurrence in ordered:
            target = EventEnrichmentTarget.model_validate(
        ...
        return {'blocks': blocks, 'event_requests': requests}
