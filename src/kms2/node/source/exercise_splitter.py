"""LangGraph node for routed source-block exercise splitting."""
...
from kms2.module.source.exercise_splitter import ExerciseSplitterModule
from kms2.module.source.exercise_strip_router import ExerciseStripRouterModule


class ExerciseSplitterWorkerState(TypedDict):
    """State supplied to one dispatched exercise splitter worker."""

    split_request: SplitRequest


class ExerciseSplitterNode:
    ...
    def dispatch(
        ...
        while flat_position < len(flat_blocks):
            ...
            if not window.target:
                raise RuntimeError(
                    'exercise splitter selected an empty target window'
                )
            sends.append(
                Send(
                    'exercise_splitter_worker',
                    {
                        'split_request': SplitRequest(
        ...
        return sends or 'exercise_splitter_collect'

    async def worker(
        self,
        state: ExerciseSplitterWorkerState,
        ...
        decisions = await self._exercise_splitter.aforward(
