r"""
Procedure extractor — fills ``content`` on each Procedure attached to a statement.

Procedures are children of StatementNode.procedures (zero or one per statement).
The extractor reads the owning statement's group text and extracts the procedure
portion. For now the concatenation is deterministic; an LLM pass will extract the
procedure portion from the full group.
"""
...
def extract_procedures(
    ...
    for sid in statement_ids:
        stmt = nodes_by_id.get(sid)
        if stmt is None or not isinstance(stmt, models.StatementNode):
            continue
        member_ids = stmt.statement_of or []
        members = [
            nodes_by_id[i] for i in member_ids if i in nodes_by_id
        ]
        text = statement_extractor.group_text(members)
        for proc in stmt.procedures:
            proc.content = text or None

    proc_count = sum(
        len(nodes_by_id[sid].procedures)
        for sid in statement_ids
        if sid in nodes_by_id
        and isinstance(nodes_by_id[sid], models.StatementNode)
    )
    logger.info(
