...
see ``graph.nodes``), the ``:Entity`` overlay on top of it (``graph.entities``), and the
``:GeneralEntity`` reference hubs (``graph.references``): a uuid uniqueness constraint on each so
