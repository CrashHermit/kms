...
see ``graph.nodes``) and the ``:Entity`` overlay on top of it (``graph.entities``): a uuid
uniqueness constraint on each so ``MERGE`` on uuid is safe and re-persisting a book never
double-inserts, plus a ``source`` lookup index on ``:Node`` and ``:Entity`` so book-scoped lookups
are efficient. Idempotent DDL (``IF NOT EXISTS``), so ``ensure_schema`` is safe to run on every
startup.
...
def schema_statements() -> list[str]:
    """The idempotent DDL for the graph: uuid uniqueness keys on ``:Node``, ``:Source`` and
    ``:Entity``, and a ``source`` lookup index on ``:Node`` and ``:Entity``."""
    return [
