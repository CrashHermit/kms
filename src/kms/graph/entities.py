...
``core.models.Entity`` onto its Neo4j form, mirroring ``graph.nodes`` for the structural layer: the
vocabulary is ``core.models.EntityType`` (it invents no new types), and it stays free of the neo4j driver
...
def entity_label(entity: models.Entity) -> str:
    """The per-type label for an entity (``models.EntityType.THEOREM`` -> ``"Theorem"``). Applied ALONGSIDE
    the base ``:Entity`` label, never instead of it. Every entity is typed, so this is never None —
    unlike ``node_label``. The models.EntityType values are single lowercase words, so capitalizing yields a
    valid Neo4j label."""
