...
Writes one vertex per ``ASTNode`` (base ``:Node`` label + its per-type label), MERGEd on the
deterministic uuid so re-running a book is idempotent, then threads them with ``:NEXT`` edges in
document order so the stream is walkable in Cypher (order is also on the ``index`` property; the
edges just make traversal natural).
...
stream into one MERGE per label plus one for the edges — no per-node round-trips. The pure
planning (grouping, edge pairs) is factored out and unit-tested; only the two ``session.run``
...
from kms.graph.nodes import NODE_LABEL, node_label, node_properties, node_uuid
...
async def persist_nodes(nodes: list[ASTNode], source: str) -> None:
    """Upsert the structural node stream and its ``:NEXT`` chain. Idempotent: MERGE on the
    deterministic uuid means re-persisting the same ``source`` updates in place. A no-op for an
    empty stream. ``source`` is the book identity; every node's id must be assigned (post-flatten)."""
    if not nodes:
        return
