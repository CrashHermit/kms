...
image / caption / header — ``core.models.NodeType``), the seam merger flattens them into the global
ordered stream, and that stream is the provenance layer every later graph tier points back to.
This module maps a ``core.models.ASTNode`` onto its Neo4j form. It invents no new node types — the
vocabulary is ``core.models.NodeType`` — and it stays free of the neo4j driver (pure mapping); the
...
``models.ASTNode.type`` for readback.
