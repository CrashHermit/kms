...
# Subsets of models.ACTIONS_ALL for the two contexts a theorem's bodylist runs in. A STATEMENT is a
...
class StatementBodylist(dspy.Signature):
    ...
    contents: str = dspy.InputField(
        description="The theorem statement's content (text + LaTeX)."
    )
...
class ProofBodylist(dspy.Signature):
    ...
    contents: str = dspy.InputField(
        description="The proof's content (text + LaTeX)."
    )
    actions: list[str] = dspy.InputField(
...
class Module(dspy.Module):
    ...
    async def statement_body(self, contents: str) -> list[models.BodySegment]:
        """Segments the theorem statement into role-labelled logical pieces."""
        result = await self.statement_bodylist.acall(
            contents=contents, actions=STATEMENT_ACTIONS
        )
        return [
            s for s in (result.bodylist or []) if s.action in STATEMENT_ACTIONS
        ]

    async def proof_body(self, contents: str) -> list[models.BodySegment]:
        """Segments the proof into role-labelled logical steps."""
        result = await self.proof_bodylist.acall(
            contents=contents, actions=PROOF_ACTIONS
        )
...
async def attribute_theorem(
    ...
    async def _statement() -> list[models.BodySegment]:
        return (
            await module.statement_body(statement_blob)
            if statement_blob
            else []
        )
...
class TheoremAttributorNode:
    ...
    async def run(self, state: state.State) -> dict:
        """Fills in each found Theorem's self-contained attributes, in place."""
        nodes_by_id = {
        ...
        if entities:
            await asyncio.gather(
                *(
                    attribute_theorem(entity, nodes_by_id, self.module)
