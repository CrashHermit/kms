...
`Refs` + `References_tactics` templates, fused into one `models.Reference` record). AutoMathKG's own example
...
is a list of `models.Reference(target, kind, tactic)`; the graph tier resolves each `target` to a
general-entity hub keyed by (kind, normalized name), so references from any book/entity converge on
one target. The tactic set is the full `models.ACTIONS_ALL`, with invalid `kind`/`tactic` values dropped.
...
class ProblemReferencerNode:
    ...
    async def run(self, state: state.State) -> dict:
        """Extracts each Problem's cross-entity references, in place."""
        entities = state.get('problem_entities', [])
        if entities:
            await asyncio.gather(
                *(reference_problem(entity, self.module) for entity in entities)
            )
