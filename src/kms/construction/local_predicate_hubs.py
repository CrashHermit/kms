"""Build source-local PredicateHub concepts from predicate components."""
...
class PredicateHubSynthesizer(module.Module):
    ...
    def decode(self, prediction, **inputs) -> tuple[str, str]:
        if not isinstance(prediction.result, PredicateHubDefinition):
            raise TypeError('result must be a PredicateHubDefinition value')
...
def _log_source_hub_diagnostics(
    ...
    if group is not None:
        details['group'] = group
    logger.info('predicate hubs: %s', details)
...
async def _rerank_ambiguous_candidates(
    ...
    *,
    ...
    if pivot_tokens >= comparison_token_budget:
        raise ValueError(
            f'predicate hubs: pivot {pivot["uuid"]!r} requires {pivot_tokens} '
    ...
    for candidate, text in candidate_texts:
        ...
        if pivot_tokens + token_cost > comparison_token_budget:
            raise ValueError(
                f'predicate hubs: candidate {candidate["uuid"]!r} with pivot '
...
PREDICATE_SPEC = PredicateHubSpec(
    ...
    hub_id_factory=lambda records, definition: graph_predicate_hubs.hub_uuid(
        _dominant_source(records),
        '|'.join(sorted(record['uuid'] for record in records)),
    ),
    source_resolver=_dominant_source,
    synthesis_context='Synthesize a source-local semantic predicate hub.',
...
def _workflow_spec() -> PredicateHubSpec:
    """Returns the graph-specific metadata for workflow predicate hubs."""
    return PredicateHubSpec(
        graph=hubs,
        stage_name=PREDICATE_SPEC.stage_name,
        hub_id_factory=lambda records, definition: hubs.hub_uuid(
            hubs.LOCAL_PREDICATE_HUB_LABEL,
            _dominant_source(records),
            '|'.join(sorted(record['uuid'] for record in records)),
        ),
...
async def build_predicate_hubs(
    records: list[dict],
    *,
    source: str | None,
    ...
    """Clusters all source-local predicate components into locked hub groups."""
    ...
    if missing:
        ...
        raise RuntimeError(
            f'predicate hubs: {len(missing)} record(s) lack an embedding — '
...
async def rebuild(
    ...
    *,
    ...
    spec = PredicateHubSpec(
        ...
        stage_name='predicate_hubs',
        hub_id_factory=source_hub_id_factory(graph_predicate_hubs),
    ...
    result = await build_predicate_hubs(
        component_rows,
        source=source,
        spec=spec,
        max_concurrency=max_concurrency,
    ...
    async def run(self, current_state: dict) -> dict:
        ...
        return {
            'predicate_hub_assignments': (
                construction_bundle.predicate_hub_assignments
            ),
            'predicate_hub_diagnostics': result['diagnostics'],
