...
def require_meta_sources(spec: EntityHubSpec, records: list[dict]) -> set[str]:
...
async def align_meta_hubs(
    ...
    *,
    ...
    spec.all_source_hubs = queries.all_entity_source_hubs
    spec.qualified_meta_hub_uuids = queries.qualified_entity_meta_hub_uuids
    spec.index_name = 'meta_entity_hub_embedding'
    spec.clear_hubs = writer.clear_entity_meta_hubs
    spec.persist_hubs = writer.persist_entity_hubs
    spec.attach_meta_hubs = writer.attach_entity_meta_hubs
    spec.clear_invalid_meta_hubs = writer.clear_invalid_entity_meta_hubs
    ...
    eligible_meta_hubs = await queries.qualified_entity_meta_hub_uuids(
        session_factory
    )
    top_k = config.get_settings().stages.search.top_k
    gate = llm.gate(max_concurrency)
    ...
    await writer.clear_invalid_entity_meta_hubs(session_factory=session_factory)
...
async def rebuild_meta(
    *,
    ...
    await writer.clear_entity_meta_hubs(session_factory=session_factory)
    await writer.persist_entity_hubs(
        result['hubs'],
        session_factory=session_factory,
