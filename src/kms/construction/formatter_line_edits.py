...
from kms.construction import formatting_stages
from kms.core import edits, models, module, state

LineEdit = edits.LineEdit
apply_line_edits = edits.apply_line_edits
number_lines = edits.number_lines
...
class Signature(dspy.Signature):
    ...
    Input:
    `[1] \(x^2\) is nonnegative`

    Output:
    `[{"index": 1, "replacement": "$x^2$ is nonnegative", "operation": "replace"}]`
    ...
    Output:
    `[{"index": 2, "replacement": "$x$ is positive", "operation": "replace"}]`

    Invalid: never invent a line number. For a one-line input, this is wrong:

    Input:
    `[1] \(x\)`

    Wrong output:
    `[{"index": 2, "replacement": "$x$", "operation": "replace"}]`

    The correct output is:
    `[{"index": 1, "replacement": "$x$", "operation": "replace"}]`

    Invalid: do not emit two edits for the same line. For `[1] \(x\)`, this
    is wrong:
    `[{"index": 1, "replacement": "$x$", "operation": "replace"}, {"index": 1, "replacement": "$x$", "operation": "replace"}]`

    Use only the line numbers present in the input. Never use an index greater
    than the input's maximum line number.
...
class Formatter:
    ...
    def __init__(self, language_model, recorder=None) -> None:
        ...
        self.stages = (
            formatting_stages.MathFormatter(language_model, recorder),
            formatting_stages.StructureFormatter(language_model, recorder),
        )

    async def aforward(self, *, node_content: str) -> str:
        formatted = node_content
        for stage in self.stages:
            formatted = await stage.aforward(node_content=formatted)
        return formatted
...
class FormatterNode:
    ...
    async def worker(self, state: dict) -> dict:
        ...
        try:
            ...
            raise ValueError(
                'formatter failed at '
                f'document={document.index}, node={node.index}: {error}'
            ) from error
